...
def new_Load_cell(length, cell_name):
	...
	## TiN (left)
	TIN_width = 13 # um
	TIN_length = 36 # um => ~74.8 Ω (Rs=27 Ω/sq) + contact resistance 8*3=24 Ω
	contact_width = 10
	contact_length = 10
...
def new_Load_PIN_cell(length, cell_name, with_end_pad=False):
	ret_cell = gdstk.Cell(cell_name)
	CPW_cell = new_CPW_cell(length+1, cell_name+"_CPW", with_end_pad=with_end_pad)
...
def new_PIN_AMZM_GC_cell(PIN_length, cell_name, with_TERM=False, with_end_pad=False):
	...
	if with_TERM:
		...
		CPW_cell = new_CPW_cell(PIN_length+1, cell_name, with_end_pad=with_end_pad)
		ret_cell.add(gdstk.Reference(CPW_cell, origin=(0, 0)))
...
LIB.write_gds("RF_calib_pattern_v5.gds")
