...
def new_PIN_MZM_cell(PIN_length, cell_name):
	...
	o = horizontal(o, -( np.abs(start_point[0]-end_point[0]) - 2*(radius+dr) ), layer, ret_cell)
	o = arc_LU(o, layer, ret_cell)
	## straight waveguide between 2x2 MMI (bottom) and taper
	STRAIGHT_LEFT_START = o.copy()
	straight_length = RF_PAD_PITCH
	o = vertical(o, straight_length, layer, ret_cell)
	...
	MET_MIDDLE_corner_botleft = [
		TAPER_BOT_LEFT[0] + 0.5 + 4,
		STRAIGHT_LEFT_START[1]
	]
	MET_MIDDLE_corner_topright = [
		TAPER_BOT_RIGHT[0] - 0.5 - 4,
		STRAIGHT_RIGHT_START[1] + RF_PAD_PITCH
	]
...
def PIN_structure(PIN_length, start_point, cell_name):
	...
	# => contact distance = 12 um
	# => contact thickness = 1.5 um, max length = 500 um
	# => SiO2 permittivity ~ 3.9 (for MHz)
	# => capacitance = ε S/d = 3.9 * 1.5e-9 * 500e-9 / 12e-9 = 244 nF
	# => RC limited frequency = 1 / (2 * 3.14 * 50Ω * 244nF) -> 1.3 kHz
	...
	layer = LAYER_MET
	o = start_point.copy()
	MET_LEFT_corner_botright = [
		CT2PN_LEFT_corner_botright[0] + 1.5,
		o[1]
	]
	MET_LEFT_corner_topleft = [
		CT2PN_LEFT_corner_topleft[0] - 1.5,
		CT2PN_LEFT_corner_topleft[1] + 2
	]
	# => metal wire width = 10 um, max length = 530 um
	# => resistance = 26mΩ / 10 * 530 = 1.4 Ω
	# => inductance ~ 0.54 nH -> impedance = 2πfL = 2 * 3.14 * 1GHz * 0.54nH = 3.4 Ω
	# => L/R constant = 0.54nH / 50Ω = 92.5 GHz
	# reference: http://gate.ruru.ne.jp/rfdn/Tools/PrintWForm.asp#p2
	MET_LEFT_rectangle = gdstk.rectangle(MET_LEFT_corner_botright, MET_LEFT_corner_topleft, layer=layer, datatype=0)
	ret_cell.add(MET_LEFT_rectangle)
	o = start_point.copy()
	MET_RIGHT_corner_botleft = [
		CT2PN_RIGHT_corner_botleft[0] - 1.5,
		o[1]
	]
	MET_RIGHT_corner_topright = [
		CT2PN_RIGHT_corner_topright[0] + 1.5,
		CT2PN_RIGHT_corner_topright[1] + 2
	]
	# => metal wire width = 10 um, max length = 530 um
	# => resistance = 26mΩ / 10 * 530 = 1.4 Ω
	# => inductance ~ 0.54 nH -> impedance = 2πfL = 2 * 3.14 * 1GHz * 0.54nH = 3.4 Ω
	# => L/R constant = 0.54nH / 50Ω = 92.5 GHz
	# reference: http://gate.ruru.ne.jp/rfdn/Tools/PrintWForm.asp#p2
	MET_RIGHT_rectangle = gdstk.rectangle(MET_RIGHT_corner_botleft, MET_RIGHT_corner_topright, layer=layer, datatype=0)
	ret_cell.add(MET_RIGHT_rectangle)
